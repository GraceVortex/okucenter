...
def test_recognize_face(request):
    ...
    if request.method == 'GET':
        # Отображаем HTML-шаблон при GET-запросе
        return render(request, 'face_recognition/test_recognize.html')
    ...
    try:
        ...
        if all_similarities:
            for user_data, score in all_similarities:
                user_scores.append({
                    ...
                    'confidence': float(round(float(score) * 100, 2))  # Преобразуем в проценты, округляем и преобразуем в Python float
                })
        ...
        if success and user:
            ...
            return JsonResponse({
                ...
                'threshold_percent': float(round(FACENET_THRESHOLD * 100, 2))
            })
        else:
            return JsonResponse({
                'success': False, 
